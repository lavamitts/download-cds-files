...
class MeasureConditionComponent(Master):
    ...
    def __init__(self, elem, measure_condition_sid):
    ...
    def get_duty_string(self):
        ...
        if self.duty_expression_id == "01":
            if self.monetary_unit_code == "":
                ...
                if self.measurement_unit_code != "":
                    ...
                    if self.measurement_unit_qualifier_code != "":
                        self.duty_string += " / " + self.measurement_unit_qualifier_code

        elif self.duty_expression_id in ("02"):
            if self.monetary_unit_code == "":
                self.duty_string += " - {0:1.4f}".format(self.duty_amount) + "%"
            else:
            ...
            if self.monetary_unit_code == "":
                ...
                if self.measurement_unit_code != "":
                    self.duty_string += " / " + \
                        f.get_measurement_unit(self.measurement_unit_code)
                    if self.measurement_unit_qualifier_code != "":
    ...
    def xget_duty_string(self):
        ...
        if self.duty_expression_id == "01":
            if self.monetary_unit_code == "":
                ...
                self.duty_string += self.monetary_unit_code + " " + "{0:1.4f}".format(self.duty_amount) + " "

        elif self.duty_expression_id in ("02"):
            if self.monetary_unit_code == "":
                self.duty_string += " minus {0:1.4f}".format(self.duty_amount) + "%"
            else:
                if self.measurement_unit_code != "":
                    self.duty_string += self.measurement_unit_code + " "
                    if self.measurement_unit_qualifier_code != "":
            ...
            print("Measure condition component: Unexpected duty expression found", self.duty_expression_id, str(self.measure_condition_sid))
